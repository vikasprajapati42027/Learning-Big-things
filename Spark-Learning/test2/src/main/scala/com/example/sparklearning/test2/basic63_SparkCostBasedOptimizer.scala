...
object basic63_SparkCostBasedOptimizer {
  def main(args: Array[String]): Unit = {
    val spark = SparkSession.builder()
      .appName("basic63_SparkCostBasedOptimizer")
      .master("local[*]")
    ...
    val a = spark.range(0, 100).withColumn("k", col("id") % 10)
    val b = spark.range(0, 20).withColumn("k", col("id") % 10)
    val c = spark.range(0, 5).withColumn("k", col("id") % 10)
